      ...
      <SidebarHeader className="p-4">
        <div className="flex items-center gap-3">
          <div className="relative h-10 w-10 overflow-hidden rounded-lg gold-glow">
            <Image src="/images/apax-logo.png" alt="APAX Logo" fill className="object-cover" />
          </div>
            ...
            <SidebarMenu>
              {mainNavItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <item.icon
                      className={`h-5 w-5 ${activeView === item.view ? 'text-[#D4AF37]' : 'text-[#888888] group-hover:text-[#C0C0C0]'}`}
                    />
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {supportItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton className="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-[#C0C0C0] transition-all hover:bg-[#1A1A1A] hover:text-[#E8E8E8]">
        ...
        <div className="glass rounded-lg p-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#888888] truncate">{displayEmail}</p>
            </div>
            <button
              type="button"
